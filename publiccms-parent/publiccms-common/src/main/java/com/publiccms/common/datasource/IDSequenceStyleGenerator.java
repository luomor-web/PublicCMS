...
public class IDSequenceStyleGenerator extends SequenceStyleGenerator {
    ...
    protected QualifiedName determineSequenceName(Properties params, JdbcEnvironment jdbcEnv, ServiceRegistry serviceRegistry) {
        ...
        if (null == tableName) {
            return super.determineSequenceName(params, jdbcEnv, serviceRegistry);
        } else {
            ...
            return new QualifiedNameParser.NameParts(catalog, schema, jdbcEnv.getIdentifierHelper()
                    .toIdentifier(CommonUtils.joinString(tableName.toUpperCase(), DEF_SEQUENCE_SUFFIX)));
        }
    }
